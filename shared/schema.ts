...
// Subscription plan types - Finalized 5-tier pricing system
export const PLAN_LIMITS = {
  free: {
    price: 0,
    displayPrice: "$0",
    limits: {
      resume_scans_per_month: 3,
      job_desc_per_month: 0,
      bulk_upload_max_batch: 0,
      skills_tests_per_month: 0,
      interview_generations_per_month: 0,
      pdf_exports_per_month: 0,
    },
    features: {
      resume_logic_basic: true,
      resume_logic_full: false,
      ai_detection_basic: false,
      ai_detection_advanced: false,
  ...
  starter: {
    ...
    displayPrice: "$24.99",
    limits: {
      resume_scans_per_month: 25,
      job_desc_per_month: 5,
      bulk_upload_max_batch: 0,
      skills_tests_per_month: 0,
      interview_generations_per_month: 0,
    ...
    features: {
      resume_logic_basic: true,
      resume_logic_full: true,
      ai_detection_basic: true,
      ai_detection_advanced: false,
      candidate_profiles: true,
      pdf_export: true,
      bulk_upload: false,
  ...
  growth: {
    ...
    displayPrice: "$49.99",
    limits: {
      resume_scans_per_month: 75,
      job_desc_per_month: 10,
      bulk_upload_max_batch: 20,
      skills_tests_per_month: 25,
      interview_generations_per_month: 50,
      pdf_exports_per_month: 75,
    },
    features: {
      resume_logic_basic: true,
      resume_logic_full: true,
      ai_detection_basic: true,
      ai_detection_advanced: true,
      candidate_profiles: true,
      pdf_export: true,
      bulk_upload: true,
      bulk_actions: true,
  ...
  pro: {
    ...
    displayPrice: "$149.99",
    limits: {
      resume_scans_per_month: 250,
      job_desc_per_month: 50,
      bulk_upload_max_batch: 50,
      skills_tests_per_month: 200,
      interview_generations_per_month: 300,
      pdf_exports_per_month: 250,
    },
    features: {
      resume_logic_basic: true,
      resume_logic_full: true,
  ...
  enterprise: {
    ...
    displayPrice: "Custom",
    limits: {
      resume_scans_per_month: -1,
      job_desc_per_month: -1,
      bulk_upload_max_batch: -1,
      skills_tests_per_month: -1,
      interview_generations_per_month: -1,
...
export type LimitKey = keyof typeof PLAN_LIMITS.free.limits;
...
// Monthly usage tracking per user - matches new plan limits
export const monthlyUsage = pgTable("monthly_usage", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").notNull(),
  period: varchar("period", { length: 7 }).notNull(), // YYYY-MM format
  resumeScansUsed: integer("resume_scans_used").notNull().default(0),
  jobDescUsed: integer("job_desc_used").notNull().default(0),
  skillsTestsUsed: integer("skills_tests_used").notNull().default(0),
  interviewGenerationsUsed: integer("interview_generations_used").notNull().default(0),
...
// Legacy usage tracking - keep for backwards compatibility
