...
import { RESUME_ANALYSIS_COPY } from "@/lib/resume-analysis-copy";
...
<body>
  ...
  <div class="name-mismatch-alert">
    <div class="name-mismatch-title">⚠️ NAME MISMATCH DETECTED</div>
    <div class="name-mismatch-text">Resume name does not match candidate name. Manual verification required.</div>
  </div>
  ` : ''}
  ...
  <div class="recommendation" style="background: ${rec.color}10; border: 1px solid ${rec.color}40;">
    ...
    <span class="recommendation-text" style="color: ${rec.color}">Suggested Follow-ups: ${rec.text}</span>
  </div>
  ${wasOverridden ? `
  ...
  <div class="section">
    ...
    <div class="authenticity-grid">
      ...
      <div class="auth-item">
        <div class="auth-label">AI-like Writing Signals</div>
    ...
    <div style="margin-top: 16px; padding: 12px; background: #faf5ff; border: 1px solid #e9d5ff; border-radius: 8px;">
      <p style="font-size: 13px; color: #7c3aed;"><strong>Suggested follow-up:</strong> ${analysis.authenticitySignals.recommendation}</p>
    </div>
    ` : ''}
  </div>
  ` : ''}
                ...
                {(result.overallRecommendation || result.pass4) && (
                  <div className={cn("p-3 rounded-lg border", 
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium">{RESUME_ANALYSIS_COPY.suggestedFollowUps}:</span>
                    ...
                    {result.overallRecommendation?.wasOverridden && (
                      <div className="mt-2 text-xs text-red-600 bg-red-50 p-2 rounded">
                        <strong>{RESUME_ANALYSIS_COPY.whyNeedsFollowUp}:</strong> {result.overallRecommendation.reason}
                      </div>
                    )}
                  </div>
                )}
        ...
        <DialogContent className="w-[95vw] max-w-4xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col p-0" data-testid="dialog-analysis-results">
          <DialogHeader className="flex-shrink-0 bg-gradient-to-r from-primary/5 via-primary/10 to-transparent p-4 sm:p-6 pb-3 sm:pb-4 border-b">
            <DialogTitle className="flex items-center gap-2 sm:gap-3 text-lg sm:text-xl">
              ...
              <div>
                <span>{RESUME_ANALYSIS_COPY.modalTitle}</span>
                {result?.selectedJob && (
          ...
          {result && (
            <>
              <div className="flex flex-wrap gap-2 sm:gap-3 px-3 sm:px-6 py-3 sm:py-4 bg-muted/30 border-b flex-shrink-0">
                ...
                )} data-testid="modal-priority-score">
                  <AlertTriangle className="h-3.5 w-3.5" />
                  <span>{RESUME_ANALYSIS_COPY.reviewPriorityLabel}: {RESUME_ANALYSIS_COPY.getPriorityLevel(result.logicScore)}</span>
                </div>
                {(result.overallRecommendation || result.pass4) && (
                    ...
                    {(result.overallRecommendation || result.pass4) && (
                      <div className={cn("p-4 rounded-lg border",
                        ...
                        <div className="flex items-center gap-2 mb-3">
                          <Flag className="h-4 w-4" />
                          <span className="font-medium">{RESUME_ANALYSIS_COPY.suggestedFollowUps}</span>
                          <Badge className="bg-yellow-600">
                            Review
                          </Badge>
                        </div>
                        ...
                        {result.pass4?.nextSteps && result.pass4.nextSteps.length > 0 && (
                          <div className="space-y-2">
                            <p className="text-xs font-medium text-muted-foreground">{RESUME_ANALYSIS_COPY.suggestedQuestions}</p>
